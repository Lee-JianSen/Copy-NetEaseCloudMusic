    ...
    <div>
        <comm-nav :title="this.xiangqing.name" v-show="false">
            ...
            borderRadius: '6px'
                        ...
                        get-container="body"
                        @click="showPopupJuBaoSecond"
                >
                    <van-cell title="举报" icon="warn-o" />

                    <van-popup
                            :close="closeshowJuBaoSecondfuc"
                            v-model="showJuBaoSecond"
                            round
                            :style="{ height: '48%', width: '86%' }"
                            get-container="body"
                    >
                        <van-cell
                                title="举报原因"
        ...
        <scroll class="content" ref="scroll" :probe-type="3" :pull-up-load="true">
            <div>
                <div style="width: 100%;height: 280px;position: relative">
                    <img
                            :src="xiangqing.picUrl"
                            style="width: 100%;height: 100%"
                            alt=""
                    />
                    <comm-nav
                            style="position: absolute;top: 0;background-color:transparent;color: white"
                    >
                        <div style="position: absolute;right: 20px;">
                            <van-icon
                                    name="share"
                            ...
                            <van-icon
                                    name="ellipsis"
                                    @click="showPopupJuBao"
                                    style="position: absolute;margin-left: 310px"
                                    size="22px"
                            />
                            <van-popup
                                    :close="closeJuBao"
                  ...
                  borderRadius: '6px'
                    ...
                    <div
                            style="position: absolute;top: 200px;font-size: 20px;font-weight: bold;color: white;margin-left: 16px"
                    >
                        {{ xiangqing.name }}
                    </div>
                </div>
                ...
                >
                        ...
                        <div style="background:white;">
                            ...
                            <div v-for="(item, index) in hots" :key="index">
                                <van-cell
                                    ...
                                    <template #label>
                    <span
                            v-for="(name, indey) in item.ar"
                            :key="indey"
                            style="margin-left: 10px"
                    ><span v-if="index !== 0">/</span>{{ name.name }}</span>
                            ...
                            <div style="margin: 20px 20px 20px 20px;display:flex;flex-direction:row;">
                                <div
                                        style="margin-left: 5px;margin-right: 5px;background:#F5F5F5;height: 130px;float: left;text-align: center;padding-top: 10px;flex: 1"
                                        v-for="(item, index) in simiarts"
                                        :key="index">
                                    <div v-if="index<=2">
                                        <img
                                                :src="item.img1v1Url"
                                                style="width: 56px;border-radius: 50px;" />
                                        <p style="font-size: 14px;height: 20px;display: block">
                                            {{ item.name }}
                                        </p>
                                        <van-button
                                                round
                    ...
                    <van-tab title="歌曲" name="b">
                        ...
                        <div style="margin-left: 10px;margin-top: 10px;margin-bottom: 6px">
                            ...
                            <div
                                    style="float: right;display: inline-block;vertical-align: middle;margin-right: 16px"
                                    @click="showPopup"
                            >
                        ...
                        <div v-for="(item, index) in hots" :key="item.id">
                            <van-cell @click="getMusicId(item.id)">
                                ...
                                <template #label>
                  <span v-for="(name, index) in item.ar" :key="index" style="margin-left: 10px">
                    <span v-if="index !== 0">/</span>{{ name.name }}
                  </span>
                                    <span>{{ ' - ' + item.al.name }}</span>
                                </template>
                                <!-- 使用 right-icon 插槽来自定义右侧图标 -->
                ...
                <span style="font-weight: bold;margin-left: 4px;font-size: 15px"
                >按发行时间排序</span
                >
                            </div>
                        ...
                        <div v-for="item in zhuanji" :key="item.id">
                                ...
                                <template #title>
                                    <div>
                                        <div style="position: relative;top: 4px;left: 14px;font-weight: bold">
                                            ...
                                            <span v-for="(mingzi,index) in item.alias" :key="index+10"
                                                  style="color: grey">({{ mingzi }})</span>
                                        </div>
                                    </div>
                                </template>
                                <template #label>
                        ...
                        <div v-for="item in mv" :key="item.id">
                            <van-card @click="getMVDetailData(item.id)">
                                <template #thumb>
                                    ...
                                    >{{Math.round((item.duration / 10000) * Math.pow(10, 1)) / Math.pow(10, 1)}}万</span
                                    >
                                </template>
                                <template #title>
                                    <div
                                            style="margin-left: 46px;border: 1px solid #DF393D;color:#DF393D;width: 22px;height: 12px;float: left;margin-top: 8px;text-align: center;line-height: 12px;border-radius: 2px"
                                    >
                                        mv
                                    </div>
                                    <div
                                            style="font-size: 15px;margin-left: 46px;white-space: pre-wrap;margin-top: 4px;margin-bottom: 6px;"
                                    >
                                        {{ item.name }}
                                    </div>
                                </template>
                                <template #tags>
                  <span style="margin-left: 46px;color: grey">{{
                    item.publishTime
                  }}</span>
                                </template>
                            </van-card>
