<template>
    <div class="captchaPage">
        <Loginnav title="手机号验证" />
        <div class="info">
            <p>您的手机号:+86 {{ this.$route.query.phone }}</p>
...
<script>
import Loginnav from '../../components/nav/login-nav'
...
export default {
  ...
  mounted () {
    // 发送验证码
    SendCaptchaAPI(this.$route.query.phone)
      .then(res => {
  ...
  methods: {
            ...
            if (res.data.code === 200) {
              ...
              RegisterAPI(
                ...
                .then(res => {
                  ...
                  LoginStatusAPI()
                    .then(res => {
                      this.$store.state.userInfo = new UserInfoModel(
                        res.data.profile
                      )
                    })
                    .catch(error => {
    ...
    onDelete () {
      this.captchaNumber = this.captchaNumber.slice(
        0,
        this.captchaNumber.length - 1
      )
    }
  },
  components: {
