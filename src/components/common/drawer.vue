<template>
    ...
    >
        <transition name="move1">
            <div ref="drawer_wrap" v-show="isShowDrawer" class="drawer-wrap">
                <!--            抽屉里的内容-->
...
export default {
  ...
  methods: {
    ...
    handleTouchMove (touch, e) {
      ...
      // console.log(touch);
      const touchMove = touch.changedTouches[0].pageX // 记录移动的位置
      // 移动位置 - 刚开始触摸的位置
      const moveResult = touchMove - this.startPos

      // 刚开始 moveX = 0
      if (this.moveX > moveResult) {
        // 判断移动
